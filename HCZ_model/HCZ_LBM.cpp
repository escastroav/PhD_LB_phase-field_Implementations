...
double LB::Psi_phi(int ix, int iy, bool UseNew){
  return p_th(phi(ix,iy,UseNew)) - Cs2*phi(ix,iy,UseNew);
}
double LB::Psi_rho(int ix, int iy, bool UseNew){
  return p_th(rho(ix,iy,UseNew)) - Cs2*phi(ix,iy,UseNew);
}
...
void LB::Init(double Ux0,double Uy0,double gx,double gy){
  double W = 5.0; double phi0,rho0,p0,gr_x0,gr_y0;
  //double Fsx0, Fsy0, Fx, Fy;
  double rhoRT;//,Ux0,Uy0;
  for(int ix=0;ix<Lx;ix++)
    for(int iy=0;iy<Ly;iy++){
      phi0 = (phi_h+phi_l)*0.5+(phi_h-phi_l)*0.5*tanh(2*(double)(iy-Ly/2)/W);
      rho0 = (rho_h+rho_l)*0.5+(rho_h-rho_l)*0.5*tanh(2*(double)(iy-Ly/2)/W);
...
void LB::Print(const char * NombreArchivo,double gx,double gy){
  ...
  int ix=0;
    for(int iy=0;iy<Ly;iy++){
      ...
      MiArchivo<<iy<<"\t"<<rho0<<"\t"<<phi0<<"\t"<<Ux0<<endl;
    }
  MiArchivo.close();
}
...
int main(void){
  ...
  for(t=0;t<tmax;t++){
    HCZ.Collision(g,0);
    //HCZ.ImposeFields(g,0);
    HCZ.Advection();
  }
  
  HCZ.Print("out2.dat",g,0);
